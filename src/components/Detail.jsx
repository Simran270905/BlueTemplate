...
export default function Detail() {
  return (
    <InviteCard>
      {/* Fixed Flowers */}
      <img src={flower} className="absolute top-0 left-0 w-40 md:w-56 pointer-events-none" alt="" />
      <img src={flower} className="absolute bottom-0 right-0 w-32 md:w-48 scale-x-[-1] scale-y-[-1] pointer-events-none" alt="" />
      ...
      <div className="relative z-10 flex flex-col items-center text-center space-y-6 w-full">
        ...
        <div className="max-w-md flex flex-col gap-4">
          <p className="uppercase text-sm md:text-base text-slate-500 tracking-wide">Dress Code</p>
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {["#9fb3c8", "#2e537c", "#7fa1bd", "#9cb8d6"].map((c, i) => (
              <div key={i} className="w-8 h-8 sm:w-10 sm:h-10 rounded-full" style={{ backgroundColor: c }}></div>
            ))}
          </div>
        ...
        <div className="max-w-md text-center">
          <p className="uppercase text-sm md:text-base text-slate-500 tracking-wide">RSVP</p>
          <p className="text-sm md:text-base text-slate-600">
            We have reserved <span className="font-bold">___ seat(s)</span> in your honor.
          </p>
          <p className="text-sm md:text-base text-slate-600">
            Kindly respond by <span className="font-semibold">June 1</span><br />
